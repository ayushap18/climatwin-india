...
.PHONY: help venv install data data-imd data-lst insat insat-real train downscale validate serve test bundle clean
...
help:
	...
	@echo "make insat     - build the INSAT LST layer (offline synthetic_demo) -> data/insat_lst.nc"
	@echo "make insat-real- fetch REAL INSAT-3D LST via MOSDAC mdapi (needs data/mosdac_config.json)"
...
insat-real:
	$(PY) -m data.ingest_insat --source real
